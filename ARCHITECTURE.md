...
Morphy currently supports heterogeneous data source types, including VictoriaMetrics, JSON-backed sources, a generic SQL datasource layer, and relational-style sample sources. The SQL layer is deliberately designed as an engine-dispatch surface rather than a DuckDB-only special case. DuckDB and SQLite are the first concrete SQL connectors because they are lightweight to ship locally and straightforward to test, but the surrounding datasource type is `sql`, not `duckdb` or `sqlite`. That keeps the architecture open to additional engines such as PostgreSQL or MySQL without forcing the rest of Morphy to learn a new datasource category for each SQL backend. What changes across domains is not the server skeleton, but the domain configuration and the local tool recipes that explain how to interpret that data.
...
For VictoriaMetrics, that discovery evidence includes the active query window, representative query results, label keys, metric hints extracted from query expressions, and a bounded sample of actual result rows. For JSON and relational sources, the evidence includes row shapes, sample keys, numeric fields, and sample rows. For SQL sources, the evidence includes the configured engine, connection summary, discovered tables and columns, row counts when available, numeric field hints, and a bounded sample query result. This allows the model to generate a domain that reflects the semantic shape of the real source instead of simply guessing based on the datasource type.
